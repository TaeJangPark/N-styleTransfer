...
"""
python image_stylization_create_dataset.py --style_files=/path/to/styleimages/*.jpg
        --output_file=/path/style_images.tfrecord --vgg_checkpoint=/path/tp/vgg_16.npz
"""
...
def main(unused_argv):
  ...
  with tf.python_io.TFRecordWriter(os.path.expanduser(FLAGS.output_file)) as writer:
    for style_label, style_file in enumerate(style_files):
      tf.logging.info('Processing style file %s: %s' % (style_label, style_file))
      print('Processing style file %s: %s' % (style_label, style_file))
      feature = {'label': _int64_feature(style_label)}

      style_image = load_np_image(style_file)
      buf = io.BytesIO()
      ...
      with tf.Graph().as_default():
        style_end_points = losses.precompute_gram_matrices(tf.expand_dims(tf.to_float(style_image), 0))
        for name, matrix in style_end_points.iteritems():
          feature[name] = _float_feature(np.asarray(matrix).flatten())
